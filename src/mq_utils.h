...
#define MQ_LOCK_SAFE(x) \
    ...
    {                       \
        ...
        {                   \
            int e = errno;  \
            std::cerr << "MQ_LOCK_SAFE to abort() @ " << __FILE__ << ":" <<__LINE__ << " in " << __FUNCTION__ << std::endl \
...
#else
			MQ_LOCK_SAFE(pthread_mutex_init(&mutex, NULL));
#endif
		}
...
#else
			MQ_LOCK_SAFE(pthread_mutex_destroy(&mutex));
#endif
		}
...
#else
			MQ_LOCK_SAFE(pthread_mutex_lock(&mutex));
#endif
		}
...
#else
			MQ_LOCK_SAFE(pthread_mutex_unlock(&mutex));
#endif
		}
...
public:
...
#else
        MQ_LOCK_SAFE(pthread_cond_init(&cond, NULL));
#endif
    }
    ...
    {
#ifndef _WIN32
        MQ_LOCK_SAFE(pthread_cond_destroy(&cond));
#endif
    }
...
#undef MICROSECONDS_PER_MILLISECOND
        ...
        else
        {
            if (usec < 0)
            {
                MQ_THROW(MQExceptionBase, "PTCond::wait only accepts positve usec (expect usec = -1 for infinity wait)");
            }
            timespec ts;
            timeval tp;
            gettimeofday(&tp, 0);
...
#else
        MQ_LOCK_SAFE(pthread_cond_signal(&cond));
#endif
    }
