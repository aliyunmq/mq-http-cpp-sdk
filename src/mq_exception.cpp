...
MQExceptionBase::MQExceptionBase(const std::string& msg) throw()
    : mMsg(msg)
    , mFile("<unknown file>")
    , mFunc("<unknown func>")
    , mLine(-1)
...
MQExceptionBase::~MQExceptionBase() throw()
{}

void MQExceptionBase::Init(const char* file, const char* func, int line)
{
    mFile = file;
    mFunc = func;
    mLine = line;
...
std::string MQExceptionBase::GetExceptionClassName() const
{
    return "MQExceptionBase";
}

const char* MQExceptionBase::what() const throw()
{
    return ToString().c_str();
}

const std::string& MQExceptionBase::ToString() const
{
    if (mWhat.empty())
    {
        stringstream sstr("");
...
std::string MQExceptionBase::GetExceptionMessage() const
{
    return mMsg;
}

#ifndef _WIN32
std::string MQExceptionBase::GetClassName() const
{
	return GetExceptionClassName();
}

std::string MQExceptionBase::GetMessage() const
{
	return GetExceptionMessage();
}
#endif

std::string MQExceptionBase::GetStackTrace() const
{
